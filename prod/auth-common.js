    ...
    $.ajax({
        ...
        success: function (logonResponse) {
            if (!logonResponse.errorId) {
                ...
                return callback('Unable to login. Please login via Nicepage', logonResponse);
            }
        },
        error: function (err) {
            return callback('Unable to login. Please login via Nicepage', err);
        },
        dataType: 'json',
    });
};
...
window.addEventListener('load', function () {
    try {
        ...
        if (typeof sendMessageOnServer === 'function') {
            sendMessageOnServer('auth.js google.accounts.id.initialize error', {error: JSON.stringify(e)});
        }
    }
});
...
window.onAfterGoogleExternalLogOn = function onAfterGoogleExternalLogOn(error, response) {
    if (error) {
        ...
        if (typeof sendMessageOnServer === 'function') {
            sendMessageOnServer('auth.js onAfterGoogleExternalLogOn error.', error);
        }
        return;
    }
...
window.processSocialFromCms = function processSocialFromCms(formAction, socketUrl, url) {
    ...
    socket.addEventListener('message', function (event) {
        ...
        try {
            ...
            console.error(e);
            if (typeof sendMessageOnServer === 'function') {
                sendMessageOnServer('auth.js processSocialFromCms Parse error', {error: JSON.stringify(e)});
            }
        }
